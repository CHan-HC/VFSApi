...
pub async fn read_file(at: &str, path: &str) -> VfsResult<ReadFileResult> {
    vfs_log_debug!(">>> read_file START: path='{}', at_len={}", path, at.len());
    ...
    let cloud_info = if !at.is_empty() {
        vfs_log_debug!("Getting cloud file info...");
        let client = HttpClient::new().await?;
        get_cloud_file_info(&client, at, path).await
    } else {
        FileInfo {
            exists: false,
            modified_time: 0,
    ...
    match (local_info.exists, cloud_info.exists) {
        (true, true) => {
            ...
            if local_info.modified_time >= cloud_info.modified_time {
                ...
                let client = HttpClient::new().await?;
                read_cloud_file_with_client(&client, at, path, &full_path).await
            }
        }
        (true, false) => {
...
async fn get_cloud_file_info(client: &HttpClient, at: &str, path: &str) -> FileInfo {
    vfs_log_debug!(">>> get_cloud_file_info: path='{}'", path);
    
    match find_file_info(client, at, path).await {
...
async fn find_file_info(client: &HttpClient, at: &str, path: &str) -> VfsResult<(String, u64)> {
...
async fn read_cloud_file_with_client(client: &HttpClient, at: &str, path: &str, local_path: &Path) -> VfsResult<ReadFileResult> {
    vfs_log_debug!(">>> read_cloud_file_with_client START: path='{}'", path);
    
    let (file_id, _) = find_file_info(client, at, path).await?;
