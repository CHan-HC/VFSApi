...
pub async fn upload_file(at: &str, path: &str) -> VfsResult<()> {
    vfs_log_debug!(">>> upload_file START: path='{}', at_len={}", path, at.len());
    ...
    let parent_folder_id = get_parent_folder_id(&client, at, path).await?;
    vfs_log_debug!("Parent folder ID: {}", parent_folder_id);
    
    upload_to_cloud(&client, at, &file_name, &content, &parent_folder_id).await?;
    
    vfs_log_debug!("<<< upload_file END: success");
    Ok(())
}

async fn get_parent_folder_id(client: &HttpClient, at: &str, path: &str) -> VfsResult<String> {
...
async fn find_or_create_folder(client: &HttpClient, at: &str, folder_name: &str, parent_id: &str) -> VfsResult<String> {
    ...
    create_folder(client, at, folder_name, parent_id).await
}
...
async fn create_folder(client: &HttpClient, at: &str, folder_name: &str, parent_id: &str) -> VfsResult<String> {
...
async fn upload_to_cloud(client: &HttpClient, at: &str, file_name: &str, content: &[u8], parent_folder_id: &str) -> VfsResult<String> {
