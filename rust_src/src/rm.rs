...
pub async fn rm_file(at: &str, path: &str) -> VfsResult<bool> {
    vfs_log_debug!(">>> rm_file START: path='{}', at_len={}", path, at.len());
    ...
    let cloud_deleted = if !at.is_empty() {
        ...
        match delete_cloud_file(at, path).await {
            Ok(deleted) => deleted,
            Err(e) => {
...
async fn delete_cloud_file(at: &str, path: &str) -> VfsResult<bool> {
    vfs_log_debug!(">>> delete_cloud_file: path='{}'", path);
    
    let client = HttpClient::new().await?;
    
    let file_id = match find_file_id(&client, at, path).await {
        Ok(id) => id,
        Err(e) => {
...
async fn find_file_id(client: &HttpClient, at: &str, path: &str) -> VfsResult<String> {
