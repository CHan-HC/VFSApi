...
extern "C" {
    ...
    int vfs_upload_file(const char* at, const char* path);
    
    int vfs_write_file(const char* at, const char* path, const unsigned char* content_ptr, size_t content_len);
    
    int vfs_rm_file(const char* at, const char* path);
    
    int vfs_mk_dir(const char* at, const char* path);
    ...
    CListFilesResult vfs_list_files(const char* at, const char* path);
    void vfs_free_list_files_result(CListFilesResult result);
    
    CReadFileResult vfs_read_file(const char* at, const char* path);
    ...
    CStatFileResult vfs_stat_file(const char* at, const char* path);
...
static napi_value ListFiles(napi_env env, napi_callback_info info)
{
    ...
    CListFilesResult result = vfs_list_files(at, path);
    delete[] at;
    delete[] path;
...
static napi_value ReadFile(napi_env env, napi_callback_info info)
{
    ...
    CReadFileResult result = vfs_read_file(at, path);
    delete[] at;
    delete[] path;
...
static napi_value UploadFile(napi_env env, napi_callback_info info)
{
    ...
    int result = vfs_upload_file(at, path);
    delete[] at;
    delete[] path;
...
static napi_value WriteFile(napi_env env, napi_callback_info info)
{
    ...
    napi_value contentArray = args[2];
    ...
    int result = vfs_write_file(at, path, content, contentLen);
    delete[] at;
    delete[] path;
    delete[] content;
...
static napi_value RmFile(napi_env env, napi_callback_info info)
{
    ...
    int result = vfs_rm_file(at, path);
    delete[] at;
    delete[] path;
...
static napi_value StatFile(napi_env env, napi_callback_info info)
{
    ...
    CStatFileResult result = vfs_stat_file(at, path);
    delete[] at;
    delete[] path;
